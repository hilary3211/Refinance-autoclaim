...
const fs = require('fs');
const { ftGetTokenMetadata,getConfig,init_env,ftGetTokensMetadata,fetchAllPools,estimateSwap,getStablePools,instantSwap } = require("@ref-finance/ref-sdk");
...
function base64ToUint8Array(base64) {
    ...
    const lookup = Object.fromEntries([...chars].map((char, index) => [char, index]));
...
async function getNearConnection(keyStore) {
  ...
  async function getOrCreateUserSubaccountmain(userIds) {
    ...
    console.log(`Creating subaccount ${subaccountId} with balance ${initialBalance}`);


    await masterAccount.createAccount(subaccountId, newKeyPair.getPublicKey(), initialBalance)
            ...
            if (Array.isArray(user.preferences) && user.preferences.length > 0) {
                ...
                for (const pref of user.preferences) {
                    try {
                        ...
                        console.log(`Claimed rewards for ${user.wallet_id} preference:`, pref);
                        ...
                        if (pref.is_active === "true") {
                            ...
                            if (balance !== null && balance > pointOneNear) {
                                if (pref.reinvest_to === "Burrow") {
                                    ...
                                    const minAmountOut = getMinAmountOut(transactionsRef[0].functionCalls[0].args.msg);
                                    const minAmountOut2 = getMinAmountOut2(transactionsRef[0].functionCalls[0].args.msg);
                                    ...
                                    const depositResult = await masterAccount.functionCall({
                                        ...
                                        args: {
                                            ...
                                            transfer_call_args : transactionsRef[1].functionCalls[0].args,
                        ...
                        console.error(`Error processing ${user.wallet_id} preference:`, pref, error);
                    }
